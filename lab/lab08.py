...
class Unit:
    ...
    def calculate_dist(self, other_unit):
        ...
        """
        dist1 = self.location[0] - other_unit.location[0]
        dist2 = self.location[1] - other_unit.location[1]
        distance = (dist1**2 + dist2**2)**0.5
        return distance
...
class Participant:
    ...
    def send_message(self, receiver, message, sender_time):
        ...
        # append sender log to sender's outbox
            ...
            sender_log = 'You send a message to ' + str(receiver.display_name)+\
            ' at '+ str(sender_time)+': ' +message
            self.outbox.append(sender_log)
        # append receiver log to receiver's inbox
            receiver_time = (sender_time - \
            (self.time_zone - receiver.time_zone)) % 24
            receiver_log ='You receive a message from '+str(self.display_name)+\
            ' at '+str(receiver_time)+': '+message
            receiver.inbox.append(receiver_log)
    ...
    def send_to_all(self, message, sender_time):
        ...
        for i in self.participant_list:
            if i != self:
                self.send_message(i,message,sender_time)
