...
def is_fair_game(*player_ranks):
    ...
    if len(player_ranks) %2 == 0:
        if max(player_ranks) - min(player_ranks) > 5:
            return False
        if max(player_ranks) - min(player_ranks) < 5:
            return True
    else:
...
'game_6': 'not a fair game'}
    ...
    """
    result = {}
    for game,lst in games.items():
        if len(lst) == 0:
            result[game] = 'not a fair game'
        elif len(lst) %2 == 0:
            if max(lst) - min(lst) >5:
                result[game] = 'not a fair game'
            else:
...
('positional_3', 'G'), ('keyword_0_L', 'O'), ('keyword_1_I', 'S')]
    ...
    """
    return [('positional_'+str(index_1),value_1) \
    for index_1,value_1 in enumerate(args)] + \
    [('keyword_'+str(index_2)+'_'+str(value_2),kwargs[value_2]) \
     for index_2,value_2 in enumerate(kwargs)]
...
'you have to be twice as smart as the person who writes the code!'}])
    ...
    2
    """
    return len([x for m in posts for x in hashtags if x in m['hashtags']])
