...
def find_good_numbers(nums):
    ...
    []
    """
    return list(filter(lambda x: x%7==0 or x%11==0, nums))
...
def increment_list(nums, amount):
    ...
    """
    if amount > 0:
        return list(map(lambda x: x+amount, nums))
    else:
        return nums
...
def binarize_matrix(matrix, threshold):
    ...
    [[0, 0, 0], [0, 0, 0], [0, 0, 0]]
    """
    return list(map(lambda x :list(map(lambda y: 1 if y>=threshold else 0,x)),matrix))
